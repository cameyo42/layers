// Ink brush
// based on code of "Yellowtail" by Golan Levin
// modified and adapted by cameyo
//*****************************************
//*****************************************
class InkBrush
{
  ...
  float jumpDx, jumpDy;
  boolean exists;
  float thickness = brushSize;

  // constructor
  InkBrush(int mw, int mh)
  {
    w = mw;
    h = mh;
    capacity = 1000;
...
//*****************************************
  float getPressureFromVelocity(float v)
  {
    ...
    return ((minP + max(0, 1.0 - v/scaleFactor)) + (damp1*oldP))*dampInv;
  }
//*****************************************
  void setPressures()
  {
...
//*****************************************
  void path2Polys()
  {
    // compute the polygons from the path of Vec3f
    if (exists)
    {
      clearPolys();
...
//*****************************************
  void smoothPoints()
  {
    ...
    final float scaleFactor  = 1.0 / (weight + 2);
    ...
    for (int i = 1; i < nPointsMinusTwo; i++) {
      ...
      center.x = (lower.x + weight*center.x + upper.x)*scaleFactor;
      center.y = (lower.y + weight*center.y + upper.y)*scaleFactor;
    }
  }
}
//*****************************************
...
class Vec3f
{
  ...
  Vec3f(float ix, float iy, float ip) { set(ix, iy, ip); }
...
void renderInkBrushPG(InkBrush SB, int w, int h, PGraphics PG)
{
  if (SB.exists)
  {
    if (SB.nPolys > 0)
    {
      Polygon polygons[] = SB.polygons;
      int crosses[] = SB.crosses;
      ...
      PG.beginShape(QUADS);
      int gnp = SB.nPolys;
      int base = (gnp > 31) ? gnp/2 : 0; // if gnp > 31 then render only half of stroke points
      for (int i=base; i<gnp; i++)
      {
        p = polygons[i];
        xpts = p.xpoints;
        ypts = p.ypoints;
